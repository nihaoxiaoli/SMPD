    ...
    def image_path_from_index(self, index):
        ...
        for mod in self.config['ref_sens_mods']: 
            if self._image_set == 'trainval':
                path = os.path.join(self._pic_path, 'kaist/kaist_train/', 
                                    indexes[0], indexes[1], mod,   
                                    indexes[2] + self._image_ext)

            elif self._image_set == 'test':
                path = os.path.join(self._pic_path, 'kaist_test/kaist_test_{}'.format(mod), '{}_{}_{}_{}.png'.format(indexes[0], indexes[1],   
                                   indexes[2], mod))
