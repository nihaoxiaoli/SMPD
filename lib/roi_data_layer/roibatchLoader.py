...
class roibatchLoader(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    data_c = torch.from_numpy(blobs['data'][0])
    data_t = torch.from_numpy(blobs['data'][1])
    im_info = torch.from_numpy(blobs['im_info'])
    # we need to random shuffle the bounding box.
    data_height, data_width = data_c.size(1), data_c.size(2)
    if self.training:
        ...
        gt_boxes_sens = torch.from_numpy(blobs['gt_boxes_sens'])
        ...
        if ratio < 1:
            ...
            padding_data_c = torch.FloatTensor(int(np.ceil(data_width / ratio)), \
                                             data_width, 3).zero_()
            padding_data_c[:data_height, :, :] = data_c[0]

            padding_data_t = torch.FloatTensor(int(np.ceil(data_width / ratio)), \
                                             data_width, 3).zero_()
            padding_data_t[:data_height, :, :] = data_t[0]

            # update im_info
            im_info[0, 0] = padding_data_c.size(0)
            # print("height %d %d \n" %(index, anchor_idx))
        elif ratio > 1:
            # this means that data_width > data_height
            # if the image need to crop.
            padding_data_c = torch.FloatTensor(data_height, \
                                             int(np.ceil(data_height * ratio)), 3).zero_()
            padding_data_c[:, :data_width, :] = data_c[0]
            padding_data_t = torch.FloatTensor(data_height, \
                                             int(np.ceil(data_height * ratio)), 3).zero_()
            padding_data_t[:, :data_width, :] = data_t[0]

            im_info[0, 1] = padding_data_c.size(1)
        else:
            trim_size = min(data_height, data_width)
            padding_data_c = torch.FloatTensor(trim_size, trim_size, 3).zero_()
            padding_data_c = data_c[0][:trim_size, :trim_size, :]
            padding_data_t = torch.FloatTensor(trim_size, trim_size, 3).zero_()
        ...
        gt_boxes_sens_padding = torch.FloatTensor(self.max_num_box, gt_boxes_sens.size(1)).zero_()
        if keep.numel() != 0:
        ...
        padding_data_c = padding_data_c.permute(2, 0, 1).contiguous()
        padding_data_t = padding_data_t.permute(2, 0, 1).contiguous()
        ...
        data_c = data_c.permute(0, 3, 1, 2).contiguous().view(3, data_height, data_width)
        data_t = data_t.permute(0, 3, 1, 2).contiguous().view(3, data_height, data_width)
        data = [data_c, data_t]

        im_info = im_info.view(3)

        # gt_boxes = torch.FloatTensor([1,1,1,1,1])
        # gt_boxes_sens = torch.FloatTensor([1,1,1,1])
        gt_boxes = torch.from_numpy(blobs['gt_boxes'])
        gt_boxes_sens = torch.from_numpy(blobs['gt_boxes_sens'])
        num_boxes = 0
